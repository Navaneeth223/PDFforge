  ...
  const handleStartConvert = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/v1/tools/pdf-to-text`,
        formData,
      ...
      <div className="min-h-screen bg-background text-foreground pt-24 pb-12 px-4">
        <div className="max-w-4xl mx-auto space-y-8 text-center">
          ...
          <p className="text-lg text-zinc-400 max-w-2xl mx-auto">
            Extract all text from your PDF into a clean .txt file.
          </p>
          <UniversalDropzone
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col pt-24 pb-12 px-4 items-center">
      <div className="glass p-8 rounded-2xl space-y-6 max-w-xl w-full">
        ...
        <p className="text-sm text-zinc-400">
          We will extract all readable text from this PDF document. Note that scanned images will not be extracted unless you use the OCR tool.
        </p>

        <div className="pt-4 flex justify-end">
          <button onClick={handleStartConvert} className="btn-primary w-full md:w-auto bg-blue-500 hover:bg-blue-600 shadow-blue-500/25">
            Extract Text
          </button>
        </div>
      </div>
